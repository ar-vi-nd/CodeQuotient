...
function keyps(event) {
    ...
    if (event.key === "Enter") {
        ...
        if (result === "" || result === "\n") {
            ...
            // hline.setAttribute("class","col-5")
            // this will create the horizontal line which consumes 5 columns of row acc to bootstrap
            // hline.setAttribute("class","col-12")
            // this will cover the entire line
            // now the reason why this line is below all the divs is because the combined total of all the columns of other divs = 12 and row length is also 12
            // so the hline will automatically be drawn in the next line

            task.appendChild(hline)
...
function checkfn(node, tgtnode) {
    ...
    console.log(node.style.textDecoration)
    console.log(tgtnode.value)
    console.log("here here here")



    if (node.children[0].style.textDecoration != "line-through") {
        // console.log(node)
        // tgtnode.setAttribute("value","1")
        tgtnode.value = "true"

        // node.setAttribute("style", "text-decoration:line-through");
        node.children[0].setAttribute("style", "text-decoration:line-through");

        console.log("insise ckfif")
        console.log(node.style.textDecoration)
    console.log(tgtnode.value)
    console.log("insise ckfif")
        ...
        node.children[0].setAttribute("style", "text-decoration:none")
...
function editfn(node) {
    ...
    let result = node.children[0].innerHTML
    ...
    node.children[1].children[0].addEventListener("keypress", function (event) {
        ...
        if (event.key === "Enter") {
            ...
            if(node.children[1].children[0].value ==""||node.children[1].children[0].value=="\n"){
                console.log("here here here here hrer")
...
function store() {
    ...
    function adddata(obje){
        ...
        let task = document.createElement("div")
            ...
            console.log("checking chkval")
            console.log(obje.chkval)
            ...
            console.log("here now heer")
            console.log(check)
            console.log(check.parentElement.parentElement)
            console.log("here now heer")
            check.setAttribute("onclick","checkfn(this.parentElement.parentElement,this)")
